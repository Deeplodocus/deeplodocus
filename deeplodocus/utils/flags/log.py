from deeplodocus.utils.flags.ext import DEEP_EXT_CSV, DEEP_EXT_LOGS
from deeplodocus.utils.flags.path import *

DEEP_LOG_NOTIFICATION = "notification"
DEEP_LOG_HISTORY_TRAIN_BATCHES = "history_train_batches"
DEEP_LOG_HISTORY_TRAIN_EPOCHS = "history_train_epochs"
DEEP_LOG_HISTORY_VALIDATION = "history_validation"

DEEP_LOGS = {DEEP_LOG_NOTIFICATION: [DEEP_PATH_NOTIFICATION, DEEP_EXT_LOGS],
             DEEP_LOG_HISTORY_TRAIN_BATCHES: [DEEP_PATH_HISTORY, DEEP_EXT_CSV],
             DEEP_LOG_HISTORY_TRAIN_EPOCHS: [DEEP_PATH_HISTORY, DEEP_EXT_CSV],
             DEEP_LOG_HISTORY_VALIDATION: [DEEP_PATH_HISTORY, DEEP_EXT_CSV]}
